...
def index(request):
    ...
    if request.user.is_authenticated():
        owned_events = Event.objects.annotate(Count('eventattendance')).all().filter(user = request.user)
        ...
        attending_events = Event.objects.annotate(Count('eventattendance')).all().filter(id__in=attending_event_ids)
